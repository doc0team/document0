...
**Next.js:** use **`@document0/next-dev`** (`withDocument0` + one `content-stamp` import) so the dev bundler invalidates your `DocsSource` module when `content/` changes (Fumadocs-style). See the **[Next.js guide](/docs/nextjs#content-hot-reload-in-development)**.
